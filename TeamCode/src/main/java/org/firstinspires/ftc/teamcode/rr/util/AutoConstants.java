package org.firstinspires.ftc.teamcode.rr.util;

import com.acmerobotics.dashboard.config.Config;

@Config
public class AutoConstants {


    /* Preload */
    public static double PRELOAD_LINE_X = 37;
    public static double PRELOAD_LINE_Y = -12;


    public static double PRELOAD_SPLINE_POLE_X = 35.5;
    public static double PRELOAD_SPLINE_POLE_Y = -7;
    public static double PRELOAD_POLE_HEADING = Math.toRadians(140);

    public static double[] SLIDE_HEIGHTS = {4.2, 3.5, 2.3, 1.5, 0};


    public static double CONE_STACK_SPLINE_X = 48.0;
    public static double CONE_STACK_SPLINE_Y = -12.25;
    public static double CONE_STACK_HEADING = Math.toRadians(0.0);

    public static double CONE_STACK_LINE_X = 65.5;
    public static double CONE_STACK_LINE_Y = -12.25;




    public static double POLE_LINE_X = 48.0;
    public static double POLE_LINE_Y = -12.5;

    public static double POLE_SPLINE_X = 35;
    public static double POLE_SPLINE_Y = -6.9;
    public static double POLE_HEADING = 127;

}
